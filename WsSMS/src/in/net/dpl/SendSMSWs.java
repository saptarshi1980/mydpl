...
@Path("/smsws")
public class SendSMSWs {

	@GET
	@Path("/send")
	@Produces(MediaType.TEXT_PLAIN)
	public String sendSMS(@Context HttpServletRequest request,@QueryParam("refNo") String refNo) {
...
try{
				...
				if(counter==1){
					...
					StringBuffer response = new StringBuffer();
						...
						while ((inputLine = in.readLine()) != null) {
							...
							in.close();
				String update="INSERT INTO sms_sent_billdesk (ref_no,msg,time_stamp) VALUES('"+refNo+"','"+msg+"',NOW())";
	...
	@Path("/sendotp")
	@Produces(MediaType.TEXT_PLAIN)
	public String sendSMSOtp(@Context HttpServletRequest request,@QueryParam("otp") String otp,@QueryParam("mobile") String mobile) {
		String userName="durgapur";
		String password="Itcell@1357";
		String senderID="THEDPL";
		final String USER_AGENT = "Mozilla/5.0";
...
try{
			...
			try{
				System.out.println("sms send function");
				Connection conn=new ConnDB().make_connection_crm();
								
					String response1="Y";
					String msg="OTP for New Connection Application is-"+otp;
					String url = "http://www.smsjust.com/sms/user/urlsms.php?username="+userName+"&pass="+password+"&senderid="+senderID+"&dest_mobileno="+mobile+"&message="+msg+"&response="+response1;
					String newurl=url.replaceAll(" ","%20");
					URL obj = new URL(newurl);
					HttpURLConnection con = (HttpURLConnection) obj.openConnection();
					con = (HttpURLConnection) obj.openConnection();
					...
					StringBuffer response = new StringBuffer();
						...
						while ((inputLine = in.readLine()) != null) {
							...
							in.close();
				String update="INSERT INTO otp (mobile,msg,otp,time_stamp) VALUES('"+mobile+"','"+msg+"','"+otp+"',NOW())";
	...
	System.out.println("Demo from send sms");	
	return "success";
	}
