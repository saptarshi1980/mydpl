 ...
 */
@WebServlet("/GetDateDownload")
public class GetDateDownload extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GetDateDownload() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			String selectQuery="select * from upload_master where date_format(upload_date,'%d-%m-%Y')='"+date+"' and head='INITIAL' ";
			...
			if(counter>0){
				...
				request.setAttribute("date", date);
				request.getRequestDispatcher("initialDownloadFile.jsp").forward(request, response);
